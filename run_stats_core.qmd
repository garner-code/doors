...
First, store the transition probabilities for later.
...
We'll also need to read in the output from our "know 4" analysis.
...
Select just the data from the train phase, and the variables that we need to analyse.
...
Now, we look at whether experiencing different rates of context changes during training (train_type) altered how people adapted to complete and partial transfer.

First, remove the switch trials. We can also add the transition probabilities from the training phase, and the "know 4" data, to the test phase data frame.
