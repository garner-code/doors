...
k4_data <- read_csv(file.path(project_path, "res", paste(paste(exp, "maggi-k4", sep = "_"), 
...
test_data <- test_data %>% 
  ...
  mutate(sub = factor(sub), 
         ...
         complete_first = case_when(complete_first==0~"pf",complete_first==1~"cf"),
         complete_first = factor(complete_first))
```
...
```{r}
test_results <- aov_ez("sub", "accuracy", test_data, within = "transfer", 
...
We don't run contrasts here, because the omnibus test doesn't reach significance.
