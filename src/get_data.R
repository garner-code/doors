get_data <- function(data_path, exp, sub, ses, train_type, apply_threshold, min_dur) {
    ...
    if (version == "pilot-data-00" || version == "pilot-data-01" || ses != "ses-learn") {
        success <- c()
        success <- rbind(
            success, file.exists(file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "task-mforage_trls.tsv", sep = "_")))
        )
        success <- rbind(
            success, file.exists(file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "task-mforage_beh.tsv", sep = "_")))
        )
    } else {
        ...
        for (h in haus) {
            success <- rbind(
                success, file.exists(file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, h, "task-mforage_trls.tsv", sep = "_")))
            )
            success <- rbind(
                success, file.exists(file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, h, "task-mforage_beh.tsv", sep = "_")))
            )
        }
    }
    ...
    if (all(success)) {
        if (version == "pilot-data-00" || version == "pilot-data-01" || ses != "ses-learn") {
            trials <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "task-mforage_trls.tsv", sep = "_")),
                header = TRUE
            )
            resps <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "task-mforage_beh.tsv", sep = "_")),
                header = TRUE
            )
        } else {
            trials <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "house-1", "task-mforage_trls.tsv", sep = "_")),
                header = TRUE
            )
            trials <- rbind(
                trials, read.table(
                  file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "house-2", "task-mforage_trls.tsv", sep = "_")),
                  header = TRUE
              )
            )

            resps_1 <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "house-1", "task-mforage_beh.tsv", sep = "_")),
                header = TRUE
            )
            resps_2 <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "house-2", "task-mforage_beh.tsv", sep = "_")),
                header = TRUE
            )
            resps_3 <- read.table(
                file.path(data_path, exp, sub, ses, "beh", paste(sub, ses, "house-9", "task-mforage_beh.tsv", sep = "_")),
                header = TRUE
            )
        }
        ...
        resps <- resps %>%
            ...
            )  #if two onsets occured back-to-back, use the second onset as the first offset
        ...
        for (i in 1:2) {
            tmp[[i]] <- resps %>%
                ...
                mutate(
                  door_cc = case_when(
                    door %in% filter(doors, context == i)$door ~
                      1, .default = 0
                )
              ) %>%
                mutate(
                  door_oc = case_when(
                    (!(door %in% filter(doors, context == i)$door) &
                      door %in% doors$door) ~ 1, .default = 0
                )
              )
        }
