...
# make the figure
ggplot() + geom_tile(
    data = doors, aes(x = xloc, y = yloc, fill = id, colour = "white"),
    ...
    scale_colour_brewer(palette = "BrBG", labels = c(tidx, "")) +
    labs(
        title = "Optimal and Observed Paths During Training", x = "Door Position (x)", y = "Door Position (y)",
        colour = "Trial"
    ) +
    theme(
